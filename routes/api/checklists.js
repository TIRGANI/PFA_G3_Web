...
router.post(
  '/:cardId',
  [ member, [check('text', 'Text is required').not().isEmpty()]],
  async (req, res) => {
...
router.get('/checklist/:boardId', async (req, res) => {
  try {
    const card = await Card.findById(req.params.cardId);
    if (!card) {
      return res.status(404).json({ msg: 'card not found' });
    ...
    const lists = [];
    for (const listId of card.lists) {
...
router.patch('/:cardId/:complete/:itemId', [ member], async (req, res) => {
  try {
    const card = await Card.findById(req.params.cardId);
    if (!card) {
      return res.status(404).json({ msg: 'Card not found' });
...
router.delete('/:cardId/:itemId', [ member], async (req, res) => {
  try {
    const card = await Card.findById(req.params.cardId);
    if (!card) {
      return res.status(404).json({ msg: 'Card not found' });
